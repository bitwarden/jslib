...
export class ApiService implements ApiServiceAbstraction {
    ...
    async getEventsCipher(id: string, start: string, end: string, token: string): Promise<ListResponse<EventResponse>> {
        const r = await this.send(
            "GET",
    ...
    async deleteRevokeSponsorship(sponsoringOrganizationId: string): Promise<void> {
        return await this.send("DELETE", "/organization/sponsorship/" + sponsoringOrganizationId, null, true, false);
    }

    async deleteRemoveSponsorship(sponsoringOrgId: string): Promise<void> {
        return await this.send("DELETE", "/organization/sponsorship/sponsored/" + sponsoringOrgId, null, true, false);
    }

    async postPreValidateSponsorshipToken(sponsorshipToken: string): Promise<boolean> {
        const r = await this.send(
            "POST",
            "/organization/sponsorship/validate-token?sponsorshipToken=" + encodeURIComponent(sponsorshipToken),
            null,
            true,
            true
        );
